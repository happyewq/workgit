...
{
    public class LineWebhook1Controller : Controller
    {
        private readonly IConfiguration _config;

        public LineWebhook1Controller(IConfiguration config)
        {
            _config = config;
        }
        ...
        {
            Console.WriteLine("📥 收到 LINE Webhook1：");
            ...
            {
                Console.WriteLine("⚠️ 無 events 陣列，Webhook1 格式錯誤！");
                return BadRequest();
            }
    ...
    ", conn);
        ...
        {
            var token = _config["LineBot:ChannelAccessToken1"];
        ...
        private async Task ReplyToLineUser(string replyToken, string message)
        {
            ...
            string channelAccessToken1 = _config["LineBot:ChannelAccessToken1"];
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", channelAccessToken1);
...
// https://workgit.onrender.com/line/webhook1
